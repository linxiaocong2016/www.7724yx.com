...
class GamePlayPaihang extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array( 'type, uid, game_id, grade, createtime, huodong_id', 'numerical', 'integerOnly' => true ),
            ...
            array( 'id, type, uid, username, game_id, game_name, score, grade, extscore, createtime, ip, city, province, unit, huodong_id', 'safe', 'on' => 'search' ),
        );
    }
     ...
     * @return type
     */
    public function getGamePaiHang($pGameID, $pScoreOrder = 0, $pType = 0, $pPageIndex = 1, $pPageSize = 10) {
        ...
        $lvOrder = "order by score desc,modifytime desc";
        if($pScoreOrder > 0)
            $lvOrder = "order by score asc,modifytime desc";
        $lvSQL = "select score,uid,username,city,createtime,nickname,head_img from (select a.*,b.nickname,b.head_img from game_play_paihang_main a left join user_baseinfo b on a.uid=b.uid where a.game_id={$pGameID} and a.type={$pType})t group by uid,username {$lvOrder} limit {$lvStartIndex},{$pPageSize}";

        Tools::print_log(array($lvSQL,$pScoreOrder,$lvOrder));
     ...
     * @return type
     */
    public function getGamePaiHangScoreIndex($pScore, $pGameID, $pOrder = 0, $pType = 0, $pHuodongID = 0) {
        $pGameID = intval($pGameID);
        $pType = intval($pType);
        ...
        $lvWhere = " and game_id={$pGameID} ";
        if($pType > 0)
            $lvWhere.=" and type={$pType}";
        if($pHuodongID > 0)
            $lvWhere.=" and huodong_id={$pHuodongID}";

        $lvOrder = "order by score desc,id desc";
        $lvSQL = "SELECT max(score) score,uid FROM game_play_paihang_main where 1=1 {$lvWhere} group by uid {$lvOrder} limit 100";
        if($pOrder>0) {
            $lvOrder = "order by score asc,id desc";
            $lvSQL = "SELECT min(score) score,uid FROM game_play_paihang_main where 1=1 {$lvWhere} group by uid {$lvOrder} limit 100";
        }

        $lvList = Yii::app()->db->createCommand($lvSQL)->queryAll();
        Tools::print_log(array( $lvSQL, $lvList ));
        foreach( $lvList as $key => $value ) {
            if(!$pOrder && intval($pScore) >= intval($value['score']))
                return $key + 1;
              if($pOrder>0 && intval($pScore) <= intval($value['score']))
                return $key + 1;
        }
        if(count($lvList) < 100)
