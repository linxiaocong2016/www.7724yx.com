...
class UserSign extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('uid, coin', 'length', 'max'=>11),
			array('create_time', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, uid, coin, create_time', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getRand(){
		unset($_SESSION['play_game_sign']);
		$sign = strval(microtime(true)*10000).uniqid().strval(rand(10000, 99999));
		$_SESSION['play_game_sign'] = $sign;
		return $sign;
	}
	...
	public function checkSign($sign){
		if(!$sign)
			return false;
		if((isset($_SESSION['play_game_sign']) && $_SESSION['play_game_sign'] == $sign)){
			unset($_SESSION['play_game_sign']);
			return true;
		}else 
			return false;
