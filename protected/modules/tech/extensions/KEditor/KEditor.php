...
class KEditor extends CWidget {
	...
	/* TEXTAREA输入框的name，必须设置。 数据类型：String */
	public $name;
	/* TEXTAREA的id，可为空 */
	public $id;
	public $model;
	...
	public static function getUploadPath() {
		...
		if (isset ( Yii::app ()->params->uploadPath )) {
                    //自己添加的路径建立
                    CFileHelper::copyDirectory(Yii::app()->basePath . "/../assets/tmp/", Yii::app()->basePath . "/../assets/tmp/");
			return Yii::getPathOfAlias ( 'webroot' ) . str_replace ( '/', DIRECTORY_SEPARATOR, Yii::app ()->params->uploadPath );
		}
		$upload_dir=Yii::app ()->getAssetmanager ()->getPublishedPath ( $dir );
		return Yii::app ()->getAssetmanager ()->getPublishedPath ( $dir ) . DIRECTORY_SEPARATOR . 'upload';
	}
	...
	public function init() {
		...
		if (YII_DEBUG)
			$cs->registerScriptFile ( $this->baseUrl . '/kindeditor.js' );
		else
			$cs->registerScriptFile ( $this->baseUrl . '/kindeditor-min.js' );
	}
	public function run() {
		$cs = Yii::app ()->getClientScript ();
