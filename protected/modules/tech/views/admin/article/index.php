...
<tr>
    ...
    <td colspan="10" align="left" style="text-align:left">
        ...
        <input type="button" value="执行操作" onclick="actionAll(this)"  />
        ...
        <SELECT name="adminId">
        	...
        	<option value="<?php echo $k?>"><?php echo $v->nikeName?>&nbsp;|&nbsp;<?php echo $v->email?></option>
        	<?php endforeach;?>
        </SELECT>
        <input type="button" value="分配" onclick="adminIdSend(this)"  />
    </td>
</tr>
...
<script>
function actionAll(obj){
	if(!confirm("是否确认执行"))return;
	var actionName=$(obj).parent("td").find("select[name='actionName']").val();
	var ids=new Array;
...
function adminIdSend(obj){
	if(!confirm("是否确认执行"))return;
	var adminId=$(obj).parent("td").find("select[name='adminId']").val();
	var ids=new Array;
	$("input[name='_selected']:checked").each(function(i){
		ids[i]=$(this).val();
	});
