...
class CommonFunction{
	...
	function rightInfo($lvCache){
		...
		$criteria->condition=" status=1 AND create_time<$tNow ";
		if(self::$isHaveArtId!=""){
	...
	function getListLeft($cid_s=null,$keyword_s=null,$page=null,$ajax=false){
		...
		$tNow=time();
		//where语句
		$where=" status=1 AND create_time<$tNow ";
		if($cid_s>0){
			$where.=" AND cid='$cid_s' ";
		}
		if($keyword_s!==""){
	...
	function listInfo($page=null){
		...
		if(empty($lvCache)||$lvCache==""){
			...
			$lvCache['seo']['title']="{$className}_7724手机游戏资讯中心";
			$lvCache['seo']['keyword']="{$className},手机游戏";
			$lvCache['seo']['descript']="7724手机游戏资讯中心提供最全面的+{$className}+资讯.";
	...
	function indexInfo($pageSize){
		...
		if(empty($lvCache)||$lvCache==""){
			...
			$lvCache['seo']['title']="7724手机游戏资讯中心";
			$lvCache['seo']['keyword']="手机游戏,新闻,攻略";
			$lvCache['seo']['descript']="7724手机游戏资讯中心提供最新最全的手机游戏新闻,手机游戏关卡攻略,手机游戏评测等.";
