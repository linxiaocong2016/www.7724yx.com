...
class CIimagelib {
	...
	function initialize($props = array())
	{
		...
		{
			$this->set_error('原图不存在source_image');
			return FALSE;	
		}
		...
		{
			$this->set_error('getimagesize函数不存在');
			return FALSE;
		}
		...
		$this->file_name_tb=$filename.$this->thumb_marker.$file_ext;
		...
		*/
		...
		{
			//原始比例
			$exd=$this->orig_width/$this->orig_height;
			$w=(int)$this->width;
			$h=(int)$this->height;
			if($w>0&&$w>=$this->orig_width){
				$this->width=$this->orig_width;
			}
			if($h>0&&$h>=$this->orig_height){
				$this->height=$this->orig_height;
			}
			if($w>0&&$w<$this->orig_width){
				$this->height=ceil($w/$exd);
			}
			if($h>0&&$h<$this->height){
				$this->height=$h;
				$this->width=ceil($this->height*$exd);
			}
	...
	function image_create_gd($path = '', $image_type = '')
	{
		...
		switch ($image_type)
		{
			case	 1 :
						...
						{
							$this->set_error(array('不支持该图片格式', 'imglib_gif_not_supported'));
							return FALSE;
						}
			...
			case 2 :
						...
						{
							$this->set_error(array('不支持该图片格式', 'imglib_jpg_not_supported'));
							return FALSE;
						}
			...
			case 3 :
						...
						{
							$this->set_error(array('不支持该图片格式', 'imglib_png_not_supported'));
							return FALSE;
						}
	...
	function image_save_gd($resource)
	{
		switch ($this->image_type)
		{
			case 1 :
						...
						{
							$this->set_error('不支持该图片格式,GIF不支持');
							return FALSE;
						}

						if ( ! @imagegif($resource, $this->full_dst_path))
						{
							$this->set_error('图片保存失败');
							return FALSE;
						}
				break;
			case 2	:
						...
						{
							$this->set_error('不支持该图片格式,JPG不支持');
							return FALSE;
						}
			...
			case 3	:
						...
						{
							$this->set_error('不支持该图片格式,PNG不支持');
							return FALSE;
						}
