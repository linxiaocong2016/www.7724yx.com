...
{ 
	...
	function actionDelete(){
		...
		$id=trim($ids,',');
		if($id){
			$model = new GamePinglun;
			$model->deleteAll('tid=10 and id in('.$id.')');
			$model->deleteAll('tid=10 and pid in('.$id.')');
		}
		$this->redirect($_SERVER['HTTP_REFERER']);
	}
	
	//这个用户在这个活动领过的礼包
	function hdlb($uid,$gid){
		$sql="select fahaostr from game_pinglun where uid='{$uid}' and gid='{$gid}'";
	...
	function actionSendpake(){
		...
		//判断是否领过同款礼包
		$gid=$pinglun_info['gid'];
		$arr=$this->hdlb($uid,$gid);
		
		if(in_array($lbid, $arr)){
			$return['m']="该评论已经领过ID:{$lbid}-礼包了！！！";
			die(json_encode($return));
		}
		...
		//修改评论标记发送
		$sql="update game_pinglun set ishave=1,fahaostr=CONCAT(fahaostr,',{$lbid}') where id='$id'";
		yii::app()->db->createCommand($sql)->query();
