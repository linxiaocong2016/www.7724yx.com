...
class GametypesController extends LController
{
	public $lvPkid;
	public $lvTable='game_types';
	public $lvPageSize=20;
	public $lvC;
	public $imgFeild=array("pic");
	function filters(){
	...
	public function actionControll(){
		...
		if($_POST){
			$data=array();
			$data['alias']=addslashes(trim($_POST["alias"]));
			$data['name']=addslashes(trim($_POST["name"]));
			$data['listorder']=(int)$_POST["listorder"];
			if($id<=0){
	...
	function upLoadOutLocatHost($file_name){
		...
		$path = "7724/gametypes" . date ( '/Y/m/d', time () );
