...
//        }
        ...
        $path = "7724/img";
        $upload_dir = $mvRootPath . $path . date('/Y', time()); // KEditor::getUploadPath() . '/' . $dir;
        if(!file_exists($upload_dir))
            mkdir($upload_dir);
        $upload_dir = $mvRootPath . $path . date('/Y/m', time());
        if(!file_exists($upload_dir))
            mkdir($upload_dir);
        $upload_dir = $mvRootPath . $path . date('/Y/m/d', time());
        if(!file_exists($upload_dir))
            mkdir($upload_dir);
...
//            mkdir($upload_dir);
        ...
        $upload_url = '/' . $path . date('/Y/m/d', time()); // KEditor::getUploadUrl() . '/' . $dir . '/' . $date;
...
//            unlink($uploadfile);       
            //
            //$originalurl=  Yii::app()->params['img_url'].$path.'/'.$msg;
            $originalurl = 'http://img.7724.com/' . $originalurl;
            echo CJSON::encode(array( 'error' => 0, 'url' => $originalurl ));
        } else {
            echo CJSON::encode(array( 'error' => 1, 'message' => '未知错误' ));
        }
