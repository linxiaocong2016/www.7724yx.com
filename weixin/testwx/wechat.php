...
class wechatCallbackapiTest {
	...
	private function getByKeyWord($postObj, $key_word) {
		...
		if($res&&$res['pinyin']){
			...
			$sql="SELECT count(1) as num FROM where online='1' and package_name like '%{$sql_key_word}%'";
			$res=$db_con->queryRow($sql);
			if($res['num']>0){
				$url="http://7724wx.open.7724.com/?package_name_s={$key_word}";
				$html.="\n\n<a href='{$url}'>领取礼包</a>";
			}
			return $this->transmitText ( $postObj, $html );
		}
		...
		return $this->transmitText ( $postObj, $answer_content);
