...
class usermodel {
	...
	function get_user_by_mobile($mobile) {
		$arr = $this->db->fetch_first("SELECT mbrs.*,token.token FROM ".UC_DBTABLEPRE."members mbrs
				inner join ".UC_DBNAME_2."ext_userinfo uinfo on uinfo.uid=mbrs.uid 
				left join ".UC_DBNAME_2."ext_user_token token on token.uid=mbrs.uid WHERE uinfo.mobile='$mobile' ");
	...
	function get_user_by_uid($uid) {
		$arr = $this->db->fetch_first("SELECT mbrs.*,token.token FROM ".UC_DBTABLEPRE."members mbrs 
				left join ".UC_DBNAME_2."ext_user_token token on token.uid=mbrs.uid WHERE mbrs.uid='$uid' ");
	...
	function get_user_by_username($username) {
		...
		$arr = $this->db->fetch_first("SELECT mbrs.*,token.token FROM ".UC_DBTABLEPRE."members mbrs 
				left join ".UC_DBNAME_2."ext_user_token token on token.uid=mbrs.uid WHERE mbrs.username='$username'");
	...
	function get_user_by_email($email) {
		$arr = $this->db->fetch_first("SELECT mbrs.*,token.token FROM ".UC_DBTABLEPRE."members mbrs 
				left join ".UC_DBNAME_2."ext_user_token token on token.uid=mbrs.uid WHERE mbrs.email='$email'");
