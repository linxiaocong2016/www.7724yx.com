...
class usercontrol extends base {
	...
	function onlogin() {
		...
		if(empty($user)) {
			$status = -1;
		} elseif($user['password'] != md5($passwordmd5.$user['salt']) && $user['password'] != $passwordmd5) {
			$status = -2;
			...
			if($user['token']==null){
				//$lvURL = "http://web.7724_i.com:8080/api/addUserToken/uid/$status/username/$username";
				$lvURL = "http://i.7724.com/api/addUserToken/uid/$status/username/$username";
				$token= Tools::getURLContent($lvURL);
			}else{
				$token=$user['token'];
			}
		...
		return array($status, $user['username'], $password, $user['email'], $merge,$token);
	}
