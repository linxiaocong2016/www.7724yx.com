...
function getContent(value, showtype)
{
    ...
    if (showtype == 'online_ph' || showtype == 'online_zx' || showtype=='list_ph_3' || showtype == 'list_zx_3')
    {
        lvContent += '<span>'+value.game_downclick+'人玩过</span></p><p class="p3"><a href="' + downurl + '">' + showDownText + '</a></p></dt>';
    }
    ...
    if (result && result.list.length > 0)
    {
        ...
        else if (showtype == "game_detail")
        {
            $.each(result.list, function(key, value) {
                ...
                lvContent += '';
                if(value.cheadpic=='')value.cheadpic=0;
                lvContent += '   <li>';
                lvContent += '                            <p class="p1"><img src="/img/head/' + value.cheadpic + '.png"></p>';
                lvContent += '                            <p class="p2">';
                lvContent += '                                <span class="span1">' + value.ctitle + '</span>';
                lvContent += '                                <span class="span2">';
                for (index = 1; index <= 5; index++) {
                    if (index <=parseInt( value.cscore))
                        lvContent += '<img src="/img/red_star.png">';
                    else
                        lvContent += '<img src="/img/gray_star.png">';
                }
                lvContent += '</span>';
                lvContent += '                                <span class="span3">' + value.ccontent + '</span>';
                lvContent += '                            </p>';
                lvContent += '                            <p class="p3">';
                lvContent += '                                <img src="/img/detail_ico7.png" onclick="replycomment('+value.pkid+');">';
                lvContent += '                                <span>' + value.ccreate_time + '</span>';
                lvContent += '                            </p>';
                if (value.reply) {
                    lvContent += '<dl>';
                    
                    for (i = 0; i < value.reply.length; i++) {
                        var v = value.reply[i];
                        lvContent += '     <dd>';
                        lvContent += '      <p class="p4"><img src="/img/head/' + v.replyheadpic + '.png"><span>' + v.replytitle + '</span></p>';
                        lvContent += '     <p class="p5 clearfix">';
                        lvContent += '     <img src="/img/mes_ico.png">';
                        lvContent += '     <span>' + v.replycontent + '</span>';
                        lvContent += '     </p>';
                        lvContent += '     </dd>';
                    }
                    lvContent += '         </dl>';
