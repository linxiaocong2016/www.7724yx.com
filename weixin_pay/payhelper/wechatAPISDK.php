 ...
 * @copyright 2014
 */
require_once ('../WxPayPubHelper/HttpClient.class.php');
...
class Wechat {
    ...
    public function getAccessToken() {
        $token_txt = 'D:/wamp/www/wxpay/key/token.txt';
        $content = "";
        if(file_exists($token_txt)){
            $content = file_get_contents($token_txt);
        }else{
        ...
        if($content){
            ...
            $token = $content_arr[0];
            $FromTime = $content_arr[1];
            $expires = $content_arr[2];
            $timestamp = time();
            if (intval($timestamp - $FromTime) > $expires) {
                return $this->InitToken();
            }
        }else{
            return $this->InitToken();
    ...
    public function InitToken() {
        $request = HttpClient::quickGet('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' . $this->wechat_config['app_id'] . '&secret=' . $this->wechat_config['app_secret'], $this->wechat_config['cacert_url']);
        $jsoninfo = json_decode($request, true);
        $token = $jsoninfo["access_token"];
        $expires = $jsoninfo["expires_in"];
        ...
        $timestamp = time();
        $fp = @fopen( 'D:/wamp/www/wxpay/key/token.txt', 'w+');
        @fwrite($fp, $token . "|||" . $timestamp . "|||" . $expires);
        return $token;
    }
