...
*/
	...
	include_once("WxPay.pub.iosweb-config.php");
error_reporting(0);
/**
 * 所有接口的基类
 */
class Common_util_pub
{
	...
	{		
        //将XML转为array        
        $array_data = json_decode(json_encode(simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_NOCDATA)), true);//处理多出的recharge_order_no订单号
        if(isset($array_data['recharge_order_no'])){
        	unset($array_data['recharge_order_no']);
        }		
		return $array_data;
	}
